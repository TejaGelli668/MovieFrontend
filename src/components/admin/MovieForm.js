...
//       // Pre-fill the local price state from the movie's data
//       const initialPrices = {};
//       (movie.theaters || []).forEach((theater) => {
//         theater.showtimes?.forEach((showtime) => {
//           initialPrices[`${theater.id}-${showtime.time}`] = showtime.price;
//         });
//       });
            ...
            for (const showtime of theater.showtimes || []) {
              try {
                ...
                for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
                  ...
                  // FIXED: Format as local ISO string to avoid timezone shift
                  const localISOTime = new Date(
                    showDate.getTime() - showDate.getTimezoneOffset() * 60000
                  ).toISOString();

                  const showData = {
                    movie: { id: movieId },
                    theater: { id: theater.id },
                    showTime: localISOTime,
                    ticketPrice: showtime.price || formData.price || 250,
                  };
